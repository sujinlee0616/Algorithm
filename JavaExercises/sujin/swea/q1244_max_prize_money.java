...
// 틀렸음. 다시 풀어야함!!! 
// 선택정렬로 해결할 수 없음. 완전탐색. DFS로 풀어야함. 

// idea) 
// - 배열의 정렬되지 않은 부분에서, max값을 찾아서, 얘를 가장 왼쪽에 두는 방식으로 생각해서 선택정렬처럼 풀 수 있을거라고 생각했음. 
// - 그런데, max값이 여러개일떄가 있음. ex) 3 2 8 8 8 
//   ==> 여러개의 max값 중 어느 것을 고르느냐에 따라서 결과가 달라지기 때문에, 선택 정렬로는 이 경우를 계산할 수 없다. 
//   ==> 모든 경우의 수를 다 살펴봐야함 ==> ★완전탐색★으로 풀어야함. 
